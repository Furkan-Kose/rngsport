...
export const createCheckoutForm = async (req, res, next) => {
  try {
    ...
    if (order.status !== "PENDING" && order.status !== "FAILED") {
      return res.status(400).json({ message: "Bu sipariş için ödeme yapılamaz" });
    }
    ...
    const basketItems = order.items.map((item) => {
      ...
      return {
        ...
        category1: item.category === "photo" ? "Fotoğraf" : item.category === "video" ? "Video" : "Komple Paket",
    ...
    if (Math.abs(calculatedTotal - order.totalPrice) > 0.01) {
      console.error(`Fiyat tutarsızlığı! Order: ${order.id}, DB: ${order.totalPrice}, Calculated: ${calculatedTotal}`);
      return res.status(500).json({ message: "Sipariş fiyatında tutarsızlık tespit edildi" });
    }
...
export const paymentCallback = async (req, res) => {
  try {
    ...
    if (!token || typeof token !== "string") {
      return res.redirect(
        `${process.env.FRONTEND_URL}/siparis-basarisiz?error=${encodeURIComponent("Token bulunamadı")}`
      );
    }
    ...
    iyzipay.checkoutForm.retrieve(
      ...
      async (err, result) => {
        if (err) {
          ...
          return res.redirect(
            `${FRONTEND_URL}/siparis-basarisiz?error=${encodeURIComponent("Ödeme doğrulanamadı")}`
          );
        }
        ...
        if (order.conversationId && result.conversationId && order.conversationId !== result.conversationId) {
          console.error(`ConversationId mismatch! Order: ${order.conversationId}, Result: ${result.conversationId}`);
          return res.redirect(
        ...
        if (result.paymentStatus === "SUCCESS") {
          ...
          if (isNaN(paidAmount) || Math.abs(paidAmount - expectedAmount) > 0.01) {
            console.error(`Tutar uyuşmazlığı! Order: ${order.id}, Expected: ${expectedAmount}, Paid: ${paidAmount}`);
            // Tutarsızlık varsa ödemeyi kabul etme ama loglama yap
            await prisma.order.update({
              where: { id: order.id },
              data: { 
                status: "FAILED",
          ...
          await prisma.order.update({
            ...
            data: {
              status: "PAID",
              paymentId: result.paymentId ? String(result.paymentId).slice(0, 100) : null,
            },
          });
